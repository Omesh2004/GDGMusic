...
export default function UploadAnalyzePage() {
  ...
  return (
    <motion.div className="min-h-screen bg-black text-white px-6 py-12">
      <motion.h1
      ...
      <div className="flex justify-center mb-8">
        <Link href="/instruments" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition">
      ...
      <motion.div className="mb-12 space-y-4 max-w-3xl mx-auto">
        ...
        <input type="file" accept=".mp3" ref={fileInputRef} onChange={handleMP3Upload} style={{ display: "none" }} />
        ...
        {selectedFile && !mp3Uploaded && (
          ...
          >
            ...
            <button onClick={() => setShowConfirmDialog(true)} className="text-green-500 hover:underline">
              Upload 📤
            </button>
            <button onClick={handlePreview} className="text-blue-400 hover:underline">
              Preview 🎵
            </button>
        ...
        {uploadedFileName && (
          <div className="flex items-center gap-4 text-sm text-gray-400">
            ...
            <button onClick={handleMP3Cut} className="text-red-500 hover:underline">
              Cancel Upload ❌
            </button>
          </div>
        )}
      ...
      <AnimatePresence>
        {showConfirmDialog && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <motion.div className="bg-gray-900 rounded-xl p-6 max-w-md w-full" layoutId="confirmDialog">
      ...
      <AnimatePresence>
        {showAudioPlayer && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <motion.div className="bg-gray-900 rounded-xl p-6 max-w-md w-full" layoutId="audioPlayer">
              <div className="flex items-center gap-4 mb-4">
                <button onClick={() => setIsPlaying(!isPlaying)} className="p-2 rounded-full hover:bg-gray-700">
                  {isPlaying ? "⏸️" : "▶️"}
                </button>
      ...
      {loading && (
        <motion.div
          className="mt-12 max-w-6xl mx-auto flex justify-center items-center"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.3 }}
        >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gray-800 p-4 rounded-lg">
                      ...
                      <p className="text-gray-300">
                        Key: <span className="text-green-400 font-bold">{analysisData.analyses.key_tempo.key}</span>
                      </p>
                      <p className="text-gray-300">
                        Tempo:{" "}
                        <span className="text-green-400 font-bold">{analysisData.analyses.key_tempo.tempo} BPM</span>
                      </p>
                      <div className="mt-4 space-y-2 text-sm">
                        <p>
                          Mel Spectrogram Mean:{" "}
                          <span className="font-medium">
                            {analysisData.analyses.instrument.features.mel_spectrogram.mean.toFixed(2)}
                          </span>
                        </p>
                        <p>
                          MFCC Mean:{" "}
                          <span className="font-medium">
                            {analysisData.analyses.instrument.features.mfcc.mean.toFixed(2)}
                          </span>
                        </p>
                      </div>
                    </div>
